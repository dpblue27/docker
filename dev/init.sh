...
config checkout 2>&1 | egrep "\s+\." | awk {'print $1'} | xargs -I{} mv {} $HOME/.config-backup/{} || true
...
echo "== Installing dein for neovim == "
pushd .
cd /tmp
curl https://raw.githubusercontent.com/Shougo/dein.vim/master/bin/installer.sh > installer.sh
sh ./installer.sh $HOME/.vim/dein
popd
echo "== Finished installing dein for neovim == "

echo "== Cloning some git repos =="
cd $HOME/git
git clone https://github.com/tmux/tmux.git
git clone --recursive https://github.com/Andersbakken/rtags.git
echo "== Finished cloning some git repos =="
